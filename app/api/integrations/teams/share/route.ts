/**
 * POST /api/integrations/teams/share
 *
 * Sends a structured Microsoft Teams Adaptive Card message, optionally with
 * a PDF download link when pdfContext is provided.
 * Falls back to mock mode when TEAMS_WEBHOOK_URL is not configured.
 */
...
export async function POST(req: NextRequest): Promise<NextResponse<ApiResponse<ShareResult>>> {
  ...
  try {
    const result = await sendTeamsMessage(payload, resolveAppUrl(req));
    return NextResponse.json({ ok: true, data: result });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to send Teams message";
    return NextResponse.json(
